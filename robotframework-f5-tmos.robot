...
Generate Token    
    ...
    Should Be Equal As Strings    ${api_response.status_code}    200
    ${api_response_json}    Convert to Dictionary    ${api_response.json()}
...
Extend Token    
    ...
    Should Be Equal As Strings    ${api_response.status_code}    200
    ${api_token_status}    Convert to Dictionary    ${api_response.json()}
...
BIG-IP iControl TokenAuth GET    
    ...
    ${api_response.json()}    Convert to Dictionary    ${api_response.json()}
...
Get LTM Virtual Server Availability State
    ...
    ${api_response}    BIG-IP iControl BasicAuth GET    bigip_host=${bigip_host}    bigip_username=${bigip_username}    bigip_password=${bigip_password}    api_uri=${api_uri}
    Should Be Equal As Strings    ${api_response.status_code}    200
    ${virtual_server_stats_dict}    Convert to Dictionary    ${api_response.json()}
...
Add Rule to Firewall Policy
    ...
    ${rule}    convert string to json    ${rule})    json
...
Create BIG-IP HA Group
    ...
    ${pools}    convert string to json    ${pools})
    ${clusters}    convert string to json    ${clusters})
    ${trunks}    convert string to json    ${trunks})
...
Load a UCS on the BIG-IP
    ...
    ${api_uri}    set variable    /mgmt/tm/sys/ucs
    ${api_response}    Run Keyword And Ignore Error    BIG-IP iControl BasicAuth POST without Verification     bigip_host=${bigip_host}    bigip_username=${bigip_username}    bigip_password=${bigip_password}    api_uri=${api_uri}    api_payload=${api_payload}
    [Return]    ${api_response}
...
Ping Host from BIG-IP
    ...
    ${ping_output}    Get from Dictionary    ${api_response_dict}    commandResult
    log    ${ping_output}
